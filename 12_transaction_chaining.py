...
while True:
    ...
    if len(utxos) == 0:
        ...
        tx_body_estimate = TransactionBody(inputs=inputs, outputs=outputs, fee=1000000, ttl=72303971)
        vk_witnesses = []
        signature = payment_skey.sign(tx_body_estimate.hash())
        vk_witnesses.append(VerificationKeyWitness(payment_skey.to_verification_key(), signature))
        
        signed_tx = Transaction(tx_body_estimate, TransactionWitnessSet(vkey_witnesses=vk_witnesses))
        ...
        tx_body_estimate = TransactionBody(inputs=inputs, outputs=outputs, fee=estimated_fee, ttl=72303971)
        vk_witnesses = []
        signature = payment_skey.sign(tx_body_estimate.hash())
        vk_witnesses.append(VerificationKeyWitness(payment_skey.to_verification_key(), signature))
